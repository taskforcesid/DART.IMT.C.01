...
(which may be a double), then reads the price per book (which again may be 
a double). The program should then display an integer, namely the largest 
number of books that you can purchase with the given amount of money.*/

#include <stdio.h>

int main (){
    double money, pricePerBook;
    scanf("%lf %lf", &money, &pricePerBook);
    
    double maxNumber = money / pricePerBook;
    int iMaxNumber = (int)maxNumber;
    printf("%d\n",iMaxNumber);
}